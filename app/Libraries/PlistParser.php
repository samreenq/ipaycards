	...
	private function parse_node() {
		...
		switch($this->name) {
			case 'data': 
				...
				return array("type" => "data", "value" => base64_decode($this->getNodeText()));
				break;
			case 'real':
				//return floatval($this->getNodeText());
				return array("type" => "real", "value" => floatval($this->getNodeText()));
				break;
			case 'string':
				//return $this->getNodeText();
				return array("type" => "string", "value" => $this->getNodeText());
				break;
			case 'integer':
				//return intval($this->getNodeText());
				return array("type" => "integer", "value" => intval($this->getNodeText()));
				break;
			case 'date':
				//return $this->getNodeText();
				return array("type" => "date", "value" => $this->getNodeText());
				break;
			case 'true':
				//return true;
			...
			case 'array':
				...
				return array("type" => "array", "value" =>$this->parse_array());
				break;
			case 'dict':
				return $this->parse_dict();
				//return array("type" => "dict", "value" =>$this->parse_dict());
				break;
			default:
