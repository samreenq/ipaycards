...
class EntityAttributeController extends EntityController {

    private $_object_identifier = "entity_attribute";
    private $_object_identifier_list = "sys_entity_attribute";
    private $_attribute_pk = "entity_attribute_id";
...
//        }
        ...
        if (isset($data->data->page->total_records) && $data->data->page->total_records > 0) {
            ...
            if (isset($paginated_ids[0])) {
                ...
                foreach ($paginated_ids as $paginated_id) {
                    ...
                    $options .= '<a class="btn btn-xs btn-default" type="button" href="' . \URL::to($this->_panelPath . $this->_assignData['module'] . '/update/' . $paginated_id->{$this->_attribute_pk}) . '?entity_type_id=' . $request->entity_type_id . '" data-toggle="tooltip" title="Update" data-original-title="Update"><i class="fa fa-pencil"></i></a>';
    ...
    public function update(Request $request, $department, $id) {
        ...
        $this->_assignData["update"] = $ex2Model
                ->where($this->_attribute_pk, "=", $id)
    ...
    private function _update(Request $request) {
        ...
        $this->_assignData["update"] = $this->apiPostRequest(\URL::to(DIR_API) . '/system/' . $this->_object_identifier . '/update', 'POST', $_POST);
    ...
    private function _selectActions($request) {
        ...
        if ($request->select_action != "" && isset($request->checked_ids[0])) {
            foreach ($request->checked_ids as $checked_id) {
                $pk = $this->_attribute_pk;
                $postData[$pk] = $checked_id;
