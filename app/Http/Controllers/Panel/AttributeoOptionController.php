...
class AttributeoOptionController extends EntityController {

    private $_object_identifier = "attribute_option";
    private $_attribute_pk = "attribute_option_id";
    ...
    private function _selectActions($request) {
        ...
        if ($request->select_action != "" && isset($request->checked_ids[0])) {
            foreach ($request->checked_ids as $checked_id) {
                $postData['attribute_set_id'] = $checked_id;
