...
class EntityTypeController extends EntityController {
    
    private $_object_identifier = "entity_type";
    private $_attribute_pk = "entity_type_id";
    ...
    public function ajaxListing(Request $request) {
        ...
        $data = CustomHelper::internalCall($request, \URL::to(DIR_API) . '/system/' . $this->_object_identifier . '/listing', 'GET', $search_columns, false);

        $this->_assignData['records'] = $data->data->{'sys_'.$this->_object_identifier.'_listing'};
    ...
    private function _selectActions($request) {
        ...
        if ($request->select_action != "" && isset($request->checked_ids[0])) {
            foreach ($request->checked_ids as $checked_id) {
                // dont remove super admin
                if($checked_id > 0) {
                    $postData[$this->_object_identifier.'_id'] = $checked_id;
                    // remove call
                    $this->apiPostRequest(\URL::to(DIR_API).'/system/'.$this->_object_identifier.'/delete','POST',$postData);
                    // wait a while
                    sleep(1);
                }
            }
