...
class RolePermissionsController extends EntityController {
    
    private $_object_identifier = "role_permission";
    private $_attribute_pk = "role_permission_map_id";
    private $_attribute_pks = "role_permission_map_ids";
    ...
    public function ajaxListing(Request $request) {
        ...
        $dg_limit = intval($request->length);
        ...
        if (isset($paginated_ids[0])) {
            ...
            foreach ($paginated_ids as $paginated_id) {
                ...
                $options .= '<a class="btn btn-xs btn-default" type="button" href="' . \URL::to(  $this->_panelPath.$this->_assignData['module'] . '/update/' . $paginated_id->{$this->_attribute_pk}) . '" data-toggle="tooltip" title="Update" data-original-title="Update"><i class="fa fa-pencil"></i></a>';
    ...
    private function _selectActions($request) {
        ...
        if ($request->select_action != "" && isset($request->checked_ids[0])) {
            foreach ($request->checked_ids as $checked_id) {
                 $postData[$this->_attribute_pks] = $checked_id;
