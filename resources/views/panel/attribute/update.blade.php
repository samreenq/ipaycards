                        ...
                        if (isset($records[0])) {
                            ...
                            foreach ($records as $record) {
								if($record->element_type=='text') $record->element_type='input';
                                 echo $fields->randerInput($record,$update->data->attribute);
...
<script type="text/javascript">
	...
	$("#use_entity_type").on("change",function(){
			show_hide("#div_entity_type_id");
	});
	
	function show_hide(ops){
		$(ops).addClass("hideme");
		if($("#use_entity_type").val()=="1"){
			$(ops).removeClass("hideme");		
		}	
	}
	show_hide($("#div_entity_type_id"));
