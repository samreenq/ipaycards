  ...
  var FormBuilder = function FormBuilder(element, options) {
    ...
    var defaults = {
      ...
      messages: {
        ...
        timeField: 'Time Field',   //newly Added
        starRating: 'Star Rating',   //newly Added
        fileUpload: 'Upload File',   //newly Added
        description: 'Help Text',
        errorregex: 'Error Regex',
    ...
    var frmbFields = [{
      ...
      label: opts.messages.starRating,
      attrs: {
        type: 'rating',
        className: 'checkbox',
        name: 'rating'
    ...
    var advFields = function advFields(values) {
      ...
      var fieldRegex = $('<div>', {
        'class': 'frm-fld regex-wrap'
      });
      $('<label/>').text(opts.messages.errorregex).appendTo(fieldRegex);
      ...
      advFields += '<div class="frm-fld name-wrap" style="display:none;"><label>' + opts.messages.name + ' <span class="required">*</span></label>';
      advFields += '<input type="text" name="name" value="' + values.name + '" class="fld-name form-control" id="title-' + lastID + '" disabled/></div>';
      advFields += '<div class="frm-fld regex-wrap"><label>' + opts.messages.errorregex + '</label>';
      advFields += '<input type="text" name="regex" value="" class="fld-regex form-control" id="regex-' + lastID + '" /></div>';
      advFields += '<input type="hidden" name="deleted" value="0" class="fld-deleted form-control" id="deleted-' + lastID + '" /></div>';
      ...
      if ($.inArray(values.type, ['checkbox', 'select', 'checkbox-group', 'date','time', 'autocomplete', 'radio-group', 'hidden','rating']) < 0) {
    ...
    var appendFieldLi = function appendFieldLi(title, field, values) {
      ...
      li +=  '<div class="frm-fld requiredtext-wrap" style="display:none;"><label>' + opts.messages.requiredtext + '</label>';
      li +=  '<input type="text" name="required-text" class="fld-requiredtext form-control" id="requiredtext-' + lastID + '" value="" /></div>';
    ...
    var selectFieldOptions = function selectFieldOptions(values, name, selected, multipleSelect) {
      ...
      field += '<input type="text" class="option-label" id="' + name+'-option-'+i + '" placeholder="' + opts.messages.optionLabelPlaceholder + '" value="' + values.label + '" />';
      field += '<input type="text" class="option-value" id="' + name+'-option-'+i + '" placeholder="' + opts.messages.optionValuePlaceholder + '" value="' + values.value + '" />';
    ...
    $sortableFields.delegate('input.required', 'click', function () {
      ...
      $(this).parents('li.form-field').find('.requiredtext-wrap').slideToggle();
      if($(this).prop( "checked" ) == false){
        $(this).parents('li.form-field').find('.fld-requiredtext').val('');
      }
    ...
    $sortableFields.delegate('.close_field', 'click', function (e) {
      e.preventDefault();
      $(this).parent('.form-field').find('.toggle-form').trigger('click');
    });
    ...
    $(document.getElementById(frmbID + '-export-xml')).click(function (e) {
      ...
      // console.log(xml);
      $('#form_xml').val(xml);
      //$('#form_xml').val(JSON.stringify(xml));
      //$('#form_xml').val(xml.replace(/\</g, "{").replace(/\>/g, "}"));
      //return false;
      // $pre.dialog({
      //   resizable: false,
      //   modal: true,
    ...
    var fieldOptions = function fieldOptions($field) {
      ...
      $('.sortable-options li', $field).each(function () {
        var $option = $(this),
            ...
            optionId = 'id="' + $('.option-value', $option).attr('id') + '"',
            selected = $('.select-option', $option).is(':checked') ? ' selected="true"' : '';
        options.push('\n\t\t\t<option' + selected + ' ' + optionValue + ' ' + optionId + ' >' + optionLabel + '</option>');
      i++;
      });
      return options.join('');
    };
          ...
          if (!($field.hasClass('moving') || $field.hasClass('disabled'))) {
            for (var att = 0; att < opts.attributes.length; att++) {
              ...
              var xmlAttrs = {
                ...
                errorregex: $('input.fld-regex', $field).val(),
                requiredtext: $('input.fld-requiredtext', $field).val(),
                maxLength: $('input.fld-max-length', $field).val(),
                role: roleVals,
                toggle: $('.checkbox-toggle', $field).is(':checked'),
                is_deleted: $('input.fld-deleted', $field).val(),
              },
