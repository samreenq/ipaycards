...
MySQL - 5.7.28-0ubuntu0.16.04.2 : Database - ipaycardsdb
*********************************************************************
*/
...
CREATE DATABASE /*!32312 IF NOT EXISTS*/`ipaycardsdb` /*!40100 DEFAULT CHARACTER SET utf8 */;
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `category_parents`(IN _category_id INT)
BEGIN
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `delete_entity_attribute_values`(IN _entity_id INT,IN _attribute_id INT ,IN _lang_identifier VARCHAR(50),IN _table VARCHAR(100) )
BEGIN
	SET @stmt = CONCAT("DELETE FROM ",_table," WHERE entity_id = ", _entity_id," AND lang_identifier = '", _lang_identifier,"' AND attribute_id = ", _attribute_id );
	PREPARE statement FROM @stmt;
	EXECUTE statement; 
END */$$
DELIMITER ;
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `delete_entity_type_data`(IN _entity_type_id INT)
BEGIN
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `get_entities_listing`(IN _entity_type_id INT, IN _attribute_code VARCHAR(250))
BEGIN
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `get_item_inventory`(IN _entity_id INT, IN _attribute_code VARCHAR(250))
BEGIN
	DECLARE _attribute_id INT(20);
	DECLARE _data_type VARCHAR(250);
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `hard_delete_entity_data`( IN _identifier VARCHAR(100))
BEGIN
	...
	DELETE FROM `sys_entity_varbinary` WHERE `entity_type_id` = _entity_type_id ;
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `soft_delete_entity_data`( IN _entity_id INT(10), IN _deleted_at DATETIME)
BEGIN
	...
	UPDATE `sys_entity_varbinary`  SET deleted_at = _deleted_at WHERE `entity_id` = _entity_id ;
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `update_category_product_count`(IN _category_id INT,  IN _count_operator VARCHAR(1))
BEGIN
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `update_entity_attribute_value`(IN _entity_id INT, IN _attribute_code VARCHAR(250), 
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `update_item_inventory`(IN _entity_id INT, IN _attribute_code VARCHAR(250), 
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `update_product_count_in_category`()
BEGIN
